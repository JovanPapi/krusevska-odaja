...
export default function initMenuStore(): Product[] {
  const result: Product[] = [];
  const ingredients = createIngredients();

  createSalads(ingredients, result);
  createGarnishAndExtras(result);
  createAppetizers(ingredients, result);
  createGrill(ingredients, result);
  createDishesToOrder(ingredients, result);
  createSpecialitiesOfHouse(ingredients, result);
  createCookedDishes(ingredients, result);
  createDesertsAndSnacks(result);
  createDrinks(result);
...
function createSalads(ingredientsFromDB: Ingredient[], result: Product[]) {
  const productsData = [
    {
      name: "Season",
  ...
  for (const prod of productsData) {
    ...
    if (prod.ingredients && prod.ingredients.length > 0) {
      product.listOfIngredients = splitIngredients(ingredientsFromDB, prod.ingredients);
    }
...
function createGarnishAndExtras(result: Product[]) {
...
function createAppetizers(ingredientsFromDB: Ingredient[], result: Product[]) {
...
function createGrill(ingredientsFromDB: Ingredient[], result: Product[]) {
...
function createDishesToOrder(ingredientsFromDB: Ingredient[], result: Product[]) {
...
function createSpecialitiesOfHouse(ingredientsFromDB: Ingredient[], result: Product[]) {
...
function createCookedDishes(ingredientsFromDB: Ingredient[], result: Product[]) {
...
function createDesertsAndSnacks(result: Product[]) {
...
function createDrinks(result: Product[]) {
  const drinks = [
    {
